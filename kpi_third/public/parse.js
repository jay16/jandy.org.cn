...
(function(){
	...
	window.KPIParse={
		...
		setChartInfo:function(item){
			...
			if(RightIndex==2){
				var subtext=goods.name+item.name+"变化"+this.format(Number(item.main_data.data)-Number(item.sub_data.data),item.main_data.format);
			}
			if(RightIndex==3){
				var subtext=goods.name+item.name+"(%)"+scale.toFixed(2);
			}
